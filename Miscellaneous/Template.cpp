    /*-----> "॥ यदा सर्व समाप्तं स्यात्, तदा क्रीडा वास्तविकम् ॥" <----*/
#include <bits/stdc++.h>
...
#define unique(a) a.erase(unique(all(a)),a.E)
...
/*----------------------------------------------------------------------------------------------------------*/ 
template <typename T> void read(vector<T>& v) { for (auto& x : v) {std::cin >> x;}}  
template <typename T> void print(const vector<T>& v) { for(const auto& x : v) { std::cout << x << " ";} std::cout << '\n';}
...
vector<ll> sieve(int n) { int*arr = new int[n + 1](); vector<ll> a; for (ll i = 2; i <= n; i++)if (arr[i] == 0) {a.push_back(i); for (ll j = i*i; j <= n; j += i)arr[j] = 1;} return a;}
vector<ll> Sieve(int n) { vector<ll> a(n+1,1); a[0] = a[1] = 0; for (ll i = 2; i <= n; i++)if (a[i] == 1) {for (ll j = i*i; j <= n; j += i) a[j] = 0;} return a; }
bool isPrime(ll n){ if(n < 2) return false; if(n==2 || n==3) return true; if( ((n%2)==0) || ((n%3)==0) ) return false; for( ll i = 3; i*i <= n; i += 2 ) { if((n%i) == 0) return false;} return true; }
...
ll ceil(ll a,ll b){ a+=(b-1); a=a/b; return a; }
/*--------------------------------------------------------------------------------------------------------------------------*/
